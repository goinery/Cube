...
export function shortSolution(
  def: Definition,
  state: PuzzleState,
): string[] | null {
  const tokens = def.primitiveMoves.flatMap((key) =>
    (def.id === 'megaminx' ? ['', "'", '2', "2'"] : ['', "'", '2']).map(
      (suffix) => key + suffix,
    ),
  );
  ...
  const fromHome = cached || new Map<string, string[]>([[signature(home), []]]);
  let layer = [{ state: home, path: [] as string[], last: -1 }];
  for (let depth = 0; !cached && depth < 2; depth++) {
    const next: typeof layer = [];
    for (const node of layer)
      for (let i = 0; i < tokens.length; i++) {
        if (node.last >= 0 && moves[node.last].key === moves[i].key) continue;
        const moved = apply(def, node.state, [tokens[i]]),
          key = signature(moved);
  ...
  cache.set(def, fromHome);
  const direct = fromHome.get(target);
  if (direct) return inverse(def, direct);
